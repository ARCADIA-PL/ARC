...
public class TachiEnhancedPhaseInvertedTriangleParticle {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (++timer >= PARTICLE_INTERVAL) {
            timer = 0;
            spawnRotatingInvertedTriangle(player);
        }
    }

    private static void spawnRotatingInvertedTriangle(Player player) {
        if (player.level.isClientSide) {
            ...
            // 倒三角形的三个顶点（初始角度：180°, 300°, 60°）
            for (int side = 0; side < 3; side++) {
                double startAngle = 2 * Math.PI * (side + 0.5) / 3 + angleOffset; // 反转角度
                double endAngle = 2 * Math.PI * (side + 1.5) / 3 + angleOffset;
