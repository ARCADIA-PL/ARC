...
public class TachiSkill extends ComboBasicAttack{
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(new TextComponent("§c§l§o连击流太刀:第二，第三段普通攻击后半部分§b具有霸体效果§c但受伤增加"));
        list.add(new TranslatableComponent("§b常态技能命中§6进入强化状态解锁三种普攻后的技能派生；§b三种派生可§6解锁对应的强化技能"));
        ...
        list.add(new TranslatableComponent("1AS消耗一层技能，若命中后续可§b消耗两层技能§c§l发动招架反击,§6若命中则标记敌人40S"));
        list.add(new TranslatableComponent("2AS消耗一层技能主动后翻滚GP,若完美闪避则退出强化状态，发动突进;且后续常态技能变为§b消耗两层技能的§c§l强化主动后翻滚GP，§b命中被标记目标则§c§l§o发动处决"));
