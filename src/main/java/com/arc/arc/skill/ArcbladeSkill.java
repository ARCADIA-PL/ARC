...
public class ArcbladeSkill extends ComboBasicAttack {
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(new TextComponent("§6§o普攻三段完美闪避追击1：连按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("发动7段空中攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("衔接普攻第四段:可按下").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("发动GP2")))));
        list.add(new TextComponent("§6§o普攻三段完美闪避追击2：连按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey3()).append(new TextComponent("发动总计14段攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("衔接普攻第五段:可按下").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("发动GP2")))));
        list.add(new TextComponent("§d普攻四段派生：居合后瞬身，可给予自身伤害吸收以及短时间霸体，可按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("§d追击二段：命中获得抗性提升，恢复一层充能；结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻五段"))));
        ...
        list.add(new TranslatableComponent("§6常态按下").append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("§b释放GP1，消耗技能向后瞬身：若为完美闪避，可使用").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("或者").append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("§c派生攻击")))));
