...
public class ArcbladeSkill extends ComboBasicAttack {
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(new TextComponent("§d普攻一段派生1：拔剑上挑，给予自身短暂的攻速提升以及霸体；,可按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("§d发动二段一文字斩击:命中获得抗性提升以及霸体，恢复两层技能,可按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻二段"))));
        ...
        list.add(new TextComponent("§6§oGP2:追击S2：共6段空中攻击，消耗技能连点").append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("释放：结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻五段"))));
        ...
        list.add(new TextComponent("§6§oGP1:追击S1：共4段，消耗技能连点").append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("释放,其中第二段可按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent(")：提前结算；结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻四段")))));
