...
public class ArcbladeSkill extends ComboBasicAttack {
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(new TextComponent("§6§oGP2:追击A2：共7段，消耗技能连点").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("释放：结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻四段"))));
        list.add(new TextComponent("§6§oGP2:追击S2：共6段空中攻击2段地面攻击，消耗技能连点").withStyle(ChatFormatting.YELLOW).append(EpicFightKeyMappings.WEAPON_INNATE_SKILL.getTranslatedKeyMessage()).append(new TextComponent("释放：结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻五段"))));
        ...
        list.add(new TextComponent("§6§o普攻三段完美闪避追击1：消耗一层技能连按").withStyle(ChatFormatting.YELLOW).append(EpicFightKeyMappings.WEAPON_INNATE_SKILL.getTranslatedKeyMessage()).append(new TextComponent("发动7段空中攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻第四段:可按下").withStyle(ChatFormatting.YELLOW).append(EpicFightKeyMappings.WEAPON_INNATE_SKILL.getTranslatedKeyMessage()).append(new TextComponent("发动GP2")))));
        list.add(new TextComponent("§6§o普攻三段完美闪避追击2：消耗两层技能连按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey3()).append(new TextComponent("发动总计14段攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻第五段:可按下").withStyle(ChatFormatting.YELLOW).append(EpicFightKeyMappings.WEAPON_INNATE_SKILL.getTranslatedKeyMessage()).append(new TextComponent("发动GP2")))));
        list.add(new TextComponent("§d普攻四段派生：瞬身居合，可给予自身伤害吸收以及短时间霸体，可按").withStyle(ChatFormatting.YELLOW).append(EpicFightKeyMappings.WEAPON_INNATE_SKILL.getTranslatedKeyMessage()).append(new TextComponent("§d追击二段：直接打出普攻第三段：命中获得抗性提升，恢复一层充能；结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻五段"))));
