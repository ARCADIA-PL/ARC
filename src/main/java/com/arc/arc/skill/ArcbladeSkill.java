...
public class ArcbladeSkill extends ComboBasicAttack {
    ...
    public List<Component> getTooltipOnItem(ItemStack itemstack, CapabilityItem cap, PlayerPatch<?> playerCap) {
        ...
        list.add(new TextComponent("§6§oGP2:追击A2：共7段，消耗两层技能点").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("释放：结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻四段"))));
        list.add(new TextComponent("§6§oGP2:追击S2：共6段空中攻击2段地面攻击，消耗两层技能点").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("释放：结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻五段"))));
        ...
        list.add(new TextComponent("§6§o普攻三段完美闪避追击1：消耗三层技能按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("发动7段空中攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻第四段:可按下").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("发动GP2")))));
        list.add(new TextComponent("§6§o普攻三段完美闪避追击2：消耗四层技能按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey3()).append(new TextComponent("发动总计10段攻击,结束后按").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey1()).append(new TextComponent("§b衔接普攻第五段:可按下").withStyle(ChatFormatting.YELLOW).append(InvincibleKeyMappings.getTranslatableKey2()).append(new TextComponent("发动GP2")))));
