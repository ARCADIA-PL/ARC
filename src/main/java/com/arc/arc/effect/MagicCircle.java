    ...
    public static void spawnHexagramParticles(Player player, Vec3 center, double radius, double ringSpeed) {
        if (player.level instanceof ServerLevel serverLevel) {
            ...
            if ((ans <= 110 && ans >= 80 && p == 1 )|| p1 >0) {
                if(p1==0) {
                    p2 =new Random().nextInt(3);    //1为七星，2为六芒星,0为八芒星
                }
                ...
                if(p1==0&&p2==2){       //控制小六芒星生成
                    ...
                    x-=ringSpeed;
                    z-=ringSpeed;
                    hexagram_coordinate=center.add(x,8,z);
                }
...
//                }
            ...
            for (int i = 0; i < ringSteps; i++) {
                ...
                for (double y = 0; y < 1.0; y += 0.2) { // 粒子柱高度为 0.8 格
                    //serverLevel.sendParticles(ParticleTypes.LANDING_LAVA, x, center.y + y, z, 1, 0, 0, 0, 0);     //岩浆粒子
